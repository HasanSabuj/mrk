    ...
    public function login($email, $password)
    {
        ...
        catch(RoundCubeException $e)
        {
            ...
            echo'<div class="alert alert-error">
              ...
              <strong>RC Error:</strong> ' . $e->getMessage().'
            </div>';
        }
        catch(Exception $e)
        {
...
$sql="select pw_2 from users where id=?";
$result=$this->db->query($sql,[$this->session->userdata('userId')])->row();
if(!empty($result->pw_2)){
    $rcuser=$this->session->userdata('userEmail');
    $rcpass=$result->pw_2;
    // set your roundcube domain path
    $rc = new RoundcubeAutoLogin('http://buynsmile.xyz/roundcube/');
    ...
    <div class="row">
        <div class="col-xs-12">
            <iframe src="http://buynsmile.xyz/roundcube" width="100%" height="600px"></iframe>
        </div>
    </div>
    <?php    
    }
}else{
    // email password not set message
    echo'<div class="alert alert-info">
          <button class="close" data-dismiss="alert">×</button>
          <strong>Info!</strong> Please setup your email credential by updating profile information.
        </div>';
}
